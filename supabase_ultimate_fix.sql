...
SELECT
  id,
  split_part(email, '@', 1), -- Имя пользователя будет взято из email (например, "zaya2009")
  'https://api.dicebear.com/9.x/avataaars/svg?seed=' || id -- Генерируем уникальную аватарку
FROM auth.users
ON CONFLICT (id) DO NOTHING; -- Не делать ничего, если профиль уже существует

-- 2. Включаем RLS для всех таблиц (на всякий случай)
...
-- 3. Сносим ВСЕ старые политики, чтобы избежать конфликтов.
...
-- Галерея (Посты) - Все видят все, добавляют свои, удаляют свои
...
-- Лайки - Все видят все, ставят свои
...
-- 5. ХРАНИЛИЩЕ (STORAGE) - Самое важное для фото и музыки
